...
Before the transfer, 17 focused Jest suites (161 tests), package/app builds, type checks and scoped lint passed. The browser integration run exposed navigation/heading issues and was interrupted when the user requested that tests be skipped. Those issues were corrected in source; neither the full integration suite nor the transferred code has a passing post-transfer test result. No further tests are run under the latest instruction. Template synchronization completed after retaining develop's conditional module registration and enabling the dispatcher dependencies in slim mode. Generators completed; the existing Node 24 OpenAPI JSON import issue triggered static fallback, which omits request/response schemas.

After removing automatic pricing, the app type check, Sales/core build, lesson catalog validation and git diff --check passed locally. Module registries were regenerated with the existing OpenAPI fallback warning; the restarted local app returned HTTP 200. Tests remain skipped by explicit request.
