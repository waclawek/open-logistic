...
import { offerDecisionSchema, transportDecisionSchema, offerItemSchema, transportDetailSchema, carrierProposalSchema, additionalLoadProposalSchema } from '../data/validators'
import { logisticsError } from '../lib/server-domain'
import { registerTransportOptimisticLockReader } from '../lib/transports'
...
export function decisionRoute(kind: 'offer' | 'transport', proposal?: 'propose_carrier' | 'propose_load') {
  if (kind === 'transport') registerTransportOptimisticLockReader()
  const schema = proposal === 'propose_carrier' ? carrierProposalSchema : proposal === 'propose_load' ? additionalLoadProposalSchema : kind === 'offer' ? offerDecisionSchema : transportDecisionSchema
  const itemSchema = kind === 'offer' ? offerItemSchema : transportDetailSchema
  async function POST(request: Request, routeContext: DecisionContext) {
    try {
      ...
      const { result } = await bus.execute<unknown, { item: unknown }>(`logistics.${kind}s.${proposal ?? 'decide'}`, { input: { ...guardedInput, id }, ctx })
