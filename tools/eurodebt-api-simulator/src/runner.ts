...
import { signWebhookPayload } from './generators/verification'
...
export function prepareBatchRequests(
  ...
  const webhookSecret =
    process.env.EURODEBT_WEBHOOK_SECRET?.trim() ||
    String((scenario.target.headers ?? {})['X-Eurodebt-Webhook-Secret'] ?? 'eurodebt-sim-secret')
  ...
  for (let i = 0; i < batch.count; i++) {
    ...
    delete headers['X-Eurodebt-Webhook-Secret']

    const isWebhook =
      mapKey.includes('/webhooks/') ||
      template === 'verification.completed' ||
      template === 'verification.rejected'
    if (isWebhook && body != null) {
      const raw = JSON.stringify(body)
      headers['X-Eurodebt-Signature'] = signWebhookPayload(raw, webhookSecret)
      headers['User-Agent'] = 'Eurodebt-Webhook/1.0'
    }
