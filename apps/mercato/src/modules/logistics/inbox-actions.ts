...
Freight facts travel next to the lines: { pickupAddress?: string, deliveryAddress?: string, pickupWindowStart?: ISO-8601 date or date-time, pickupWindowEnd?: ISO-8601 date or date-time, deliveryWindowStart?: ISO-8601 date or date-time, cargoPallets?: number, cargoWeightKg?: number }
This seller sells road freight. One line per service asked for: per-pallet groupage, per-shipment full truckload, per-kilometre line haul, and accessorials such as a tail-lift delivery. A tail-lift, a no-dock delivery or a ground-level handover is its OWN line with quantity = number of stops.`,
    promptRules: [
      ...
      'For draft_offer: always fill pickupAddress, deliveryAddress, pickupWindowStart, pickupWindowEnd, deliveryWindowStart, cargoPallets and cargoWeightKg from what the email states. They are what turns the accepted offer into a dispatchable transport. Use an empty string, or 0 for a number, when the email does not say — never guess a city, a date or a weight.',
    ],
