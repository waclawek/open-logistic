...
import { jobCancelSchema, cancelJobBodySchema, uuidSchema } from '../../../../data/validators'
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const input = { ...cancelJobBodySchema.parse(await readJsonSafe(req, null)), id }
    const action = 'logistics.jobs.cancel'
    ...
    const guardedInput = jobCancelSchema.parse(guard.modifiedPayload ?? input)
...
export const openApi: OpenApiRouteDoc = { tag: 'Logistics', methods: { POST: {
  summary: 'Cancel an unassigned draft or ready job with a reason', requestBody: { schema: cancelJobBodySchema },
