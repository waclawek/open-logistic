...
import {
  listOpenOffers,
  publishCarrierSearch,
  searchFreeVehicles,
  searchBackloadsAlongRoute,
} from './exchange'
...
function capacitySnapshot(
  ...
  TruckPosition,
  'usedWeightT' | 'freeWeightT' | 'usedLdm' | 'freeLdm' | 'maxWeightT' | 'maxLdm'
> {
...
export function listTransportRuns(limit = 20): TransportRun[] {
  return getStore().items.slice(0, Math.min(Math.max(limit, 1), MAX))
}

export function getTransportRun(id: string): TransportRun | null {
  const run = getStore().items.find((r) => r.id === id) ?? null
  if (run && !run.stageEnteredAt) run.stageEnteredAt = run.updatedAt || run.createdAt
  return run
}

export function clearTransportRuns(): number {
...
export async function startTransportRunFromAgreedOffer(input?: {
  ...
  if (input?.withOrder !== false) {
    const order = await createOrder({
      ...
      capacity: {
        maxWeightT: input?.capacity?.maxWeightT ?? 24,
        maxLdm: input?.capacity?.maxLdm ?? 13.6,
        usedWeightT: input?.capacity?.usedWeightT ?? agreedOffer.lane.weightT,
        usedLdm: input?.capacity?.usedLdm ?? agreedOffer.lane.ldm ?? 8,
      },
      commercials: {
  ...
  const run: TransportRun = {
    ...
    agreedOffer,
    orderId,
    sourceTransportId: input?.sourceTransportId ?? null,
...
export function publishListingForRun(
  runId: string,
  listingCopy: { title: string; body: string },
): TransportRun {
  const run = requireRun(runId)
  assertStatus(run, ['awaiting_carrier_search', 'carrier_proposal_pending'])
...
export function proposeCarrier(
  ...
  if (input.source === 'active_vehicle_search') {
    const vehicle =
      run.vehicleHits.find((v) => v.id === input.vehicleId) ?? run.vehicleHits[0]
    if (!vehicle) {
    ...
    proposal = {
      ...
      summary:
        input.summary ??
        `${party.companyName} · ${vehicle.vehicleType} @ ${vehicle.locality} (${vehicle.capacityT}t)`,
      rationale,
    ...
    proposal = {
      ...
      summary:
        input.summary ??
        `${party.companyName} responded to listing ${run.listing.reference} · ${run.listing.weightT}t`,
      rationale,
...
export function approveCarrierProposal(
  runId: string,
  approvedBy = 'human2',
): TransportRun {
  const run = requireRun(runId)
...
export function scanBackloadsAlongRun(
  ...
  const aheadCandidates = search.candidates.filter(
    (c) =>
      c.kind === 'freight' &&
      c.alongRouteKm >= along &&
      c.economics.fitsFreeCapacity,
  )
...
export function updateBackloadScanProgress(
  runId: string,
  progressPct: number,
): TransportRun {
  const run = requireRun(runId)
...
export function advanceTruckAndScanBackloads(
  ...
  return { run: scanned.run, aheadCandidates: scanned.aheadCandidates, position: scanned.run.truck! }
}

export function proposeBackload(
  runId: string,
  candidateId: string,
  evaluation: string,
): TransportRun {
...
export function appendAgentLog(
  ...
  run.agentLog.unshift({ at: new Date().toISOString(), agent, text: trimmed.slice(0, 4000) })
...
export function forceCompleteAgentStage(runId: string): {
  ...
  if (run.status === 'awaiting_carrier_search') {
    ...
    if (!still.carrierProposal) {
      ...
      const price =
        Math.round(still.agreedOffer.quoteNetEur * 0.72) || still.agreedOffer.quoteNetEur
    ...
    return { run: requireRun(runId), forced: true, stage: 'awaiting_carrier_search' }
  }
  ...
  if (run.status === 'approved') {
    ...
    appendAgentLog(
      runId,
      'carrier_finder',
      'Start dostawy po limicie czasu etapu — ciężarówka na trasie GraphHopper.',
    )
  ...
  if (run.status === 'in_transit' && run.backloadScan.status === 'idle') {
    ...
    const best = aheadCandidates
      .slice()
      .sort((a, b) => b.economics.netEur - a.economics.netEur)[0]
    if (best) {
      proposeBackload(
        runId,
        best.id,
      ...
      appendAgentLog(
        runId,
        'load_optimizer',
        'Skan korytarza zakończony — brak opłacalnego doładunku z przodu.',
      )
    }
...
export function truckMarker(run: TransportRun): LatLng | null {
  ...
  return { lat: run.truck.lat, lng: run.truck.lng, name: `Truck ${run.truck.progressPct}%` }
}
