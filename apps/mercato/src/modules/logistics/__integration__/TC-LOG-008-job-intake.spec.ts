...
test('draft CRUD, version conflicts, concurrent acceptance and receipt recovery use one scoped job', async ({ logistics }) => {
  await logistics.grant(writeFeatures)
  const name = `QA intake ${randomUUID()}`
  const companyResponse = await logistics.api.post('/api/customers/companies', { data: { displayName: name } })
  ...
  const createdResponse = await logistics.api.post('/api/logistics/jobs', { data: input })
  expect(createdResponse.status()).toBe(201)
  const created = createdSchema.parse(await readJsonSafe(createdResponse))
  const replayResponse = await logistics.api.post('/api/logistics/jobs', { data: input })
  expect(replayResponse.status()).toBe(201)
  expect(createdSchema.parse(await readJsonSafe(replayResponse))).toEqual(created)
  const listed = await logistics.api.get(`/api/logistics/jobs?id=${created.id}`)
  ...
  const updatedResponse = await logistics.api.put('/api/logistics/jobs', { data: edit, headers: editHeaders })
  expect(updatedResponse.status()).toBe(200)
  const updated = createdSchema.parse(await readJsonSafe(updatedResponse))
  expect(updated.updatedAt).not.toBe(created.updatedAt)
  const replayUpdate = await logistics.api.put('/api/logistics/jobs', { data: edit, headers: editHeaders })
  expect(replayUpdate.status()).toBe(200)
  expect(createdSchema.parse(await readJsonSafe(replayUpdate))).toEqual(updated)
  expect((await logistics.api.put('/api/logistics/jobs', { data: { ...edit, cargoDescription: 'Changed reuse' }, headers: editHeaders })).status()).toBe(409)
  const stale = await logistics.api.put('/api/logistics/jobs', { data: { ...edit, requestId: randomUUID() } })
  ...
  const responses = await Promise.all([logistics.api.post(path, { data: acceptance }), logistics.api.post(path, { data: acceptance })])
  ...
  const receiptResponse = await logistics.api.get(`/api/logistics/commands/${acceptance.requestId}?action=logistics.jobs.accept`)
  expect(receiptResponse.status()).toBe(200)
  expect(await readJsonSafe(receiptResponse)).toEqual({ committed: true, result: outcomes[0] })
  const currentResponse = await logistics.api.get(`/api/logistics/jobs?id=${created.id}`)
  ...
  expect((await logistics.api.put('/api/logistics/jobs', { data: { ...edit, requestId: randomUUID(), expectedUpdatedAt: current.updatedAt } })).status()).toBe(409)
...
test('rejects protected input, invalid cargo, all-organizations selection and missing receipts safely', async ({ page, logistics, baseURL }) => {
  await logistics.grant(writeFeatures)
  expect((await logistics.api.post('/api/logistics/jobs', { data: { requestId: randomUUID(), status: 'ready' } })).status()).toBe(400)
  const requestId = randomUUID()
  const missing = await logistics.api.get(`/api/logistics/commands/${requestId}?action=logistics.jobs.accept`)
...
test('round-trips decimal drafts through persisted undo/redo and rejects later ABA edits', async ({ logistics }) => {
  ...
  const customer = await logistics.api.post('/api/customers/companies', { data: { displayName: `QA undo ${randomUUID()}` } })
  ...
  const current = async () => {
    const response = await logistics.api.get(`/api/logistics/jobs?id=${created.id}`)
  ...
  for (let cycle = 0; cycle < 2; cycle++) {
    ...
    const undone = await logistics.api.post('/api/audit_logs/audit-logs/actions/undo', { data: { undoToken: operation!.undoToken } })
    expect(undone.status()).toBe(200)
    expect(await current()).toBeUndefined()
    const redone = await logistics.api.post('/api/audit_logs/audit-logs/actions/redo', { data: { logId: operation!.id } })
  ...
  const edit = async (cargoDescription: string) => {
    ...
    return logistics.api.put('/api/logistics/jobs', { data: { ...input, requestId: randomUUID(), id: created.id, expectedUpdatedAt: record.updatedAt, cargoDescription },
  ...
  expect((await logistics.api.post('/api/audit_logs/audit-logs/actions/undo', { data: { undoToken: editOperation.undoToken } })).status()).toBe(200)
