...
const listRuns = defineAiTool({
  ...
  async handler(args) {
    const items = listTransportRuns(args.limit).map((r) => ({
      id: r.id,
      status: r.status,
...
const getRun = defineAiTool({
  ...
  async handler(args) {
    const run = getTransportRun(args.runId)
...
const searchVehicles = defineAiTool({
  ...
  description:
    'ACTIVE path: search mock exchange free vehicles near the corridor midpoint.',
  isMutation: false,
  requiredFeatures: [...FEATURES],
  inputSchema: z.object({
    runId: z.string().uuid(),
...
const publishListing = defineAiTool({
  ...
  inputSchema: z.object({
    ...
    body: z
      .string()
      .min(40)
      .describe('Full ogłoszenie: lane, weight, windows, requirements, contact ask'),
  }),
  async handler(args) {
    const run = publishListingForRun(args.runId, { title: args.title, body: args.body })
    return {
      runId: run.id,
      listing: run.listing,
...
const approveCarrierTool = defineAiTool({
  ...
  async handler(args) {
    ...
    return { runId: run.id, status: run.status, approvedCarrier: run.approvedCarrier }
  },
})
