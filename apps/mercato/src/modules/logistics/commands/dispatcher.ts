    ...
    async execute(input, ctx) {
      ...
      const result = await em.transactional(async (transaction) => {
        if (action === 'create') {
          ...
          const parsed = transportCreateSchema.parse(input)
...
export const dispatcherCommands = (['offer', 'transport'] as const).flatMap((kind) => (['create', 'delete', 'decide'] as const).map((action) => handler(kind, action)))
